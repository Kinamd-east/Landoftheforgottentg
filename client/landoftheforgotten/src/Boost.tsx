...
function Boost() {
  ...
  useEffect(() => {
    ...
    const storedTapBoostsUsed = parseInt(
      localStorage.getItem("freeBoostsUsed") || "0"
    );
    setFreeTapBoostsUsed(storedTapBoostsUsed);
    const lastBoostTime = parseInt(
      localStorage.getItem("lastBoostTime") || "0"
    );
    if (lastBoostTime) {
  ...
  useEffect(() => {
    const storedBoostsUsed = parseInt(
      localStorage.getItem("freeBoostsUsed") || "0"
    );
  ...
  useEffect(() => {
    ...
    if (storedActivationTime && storedPlanDuration) {
      ...
      const planDurationInMs =
        parseInt(storedPlanDuration, 10) * 24 * 60 * 60 * 1000;
  ...
  const handleEnergyUpgrade = async () => {
    if (petals >= energyUpgradePrice) {
      ...
      await updateDoc(doc(db, "users", uid), {
        ...
        energy: newMaxEnergy,
        energyUpgradePrice: newEnergyUpgradePrice,
        petals: newPetals,
      });
    } else {
  ...
  const handleFreeBoost = async () => {
    if (isCooldown) {
      alert(
        "You have used all your free boosts. Please wait 24 hours before using it again."
      );
      return;
    }
  ...
  const activateTapBoost = async () => {
    ...
    if (tapBoostCooldown) {
      alert(
        "You have used all your free tapping boosts. Please wait 24 hours before using it again."
      );
      return;
    }
    ...
    if (!isTapBoostActive) {
      ...
      localStorage.setItem(
        "freeTapBoostsUsed",
        updatedFreeTapBoostsUsed.toString()
      );
      ...
      setTimeout(async () => {
        ...
        await updateDoc(doc(db, "users", uid), {
          boostLevel: originalBoostLevel,
        });
        console.log(
          "Tap boost has ended, boost level reverted to 0",
          originalBoostLevel
        );
          ...
          <div className="free-boost mt-10" id="firstBoost">
            ...
            <div className=" mb-10">
              <button
                type="button"
                className="mb-2 me-2 w-full rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
                onClick={activateTapBoost}
              >
                Free Taps Boost (3 uses)
              </button>
            </div>
          </div>
              ...
              <div className="">
                <div className="text-center p-5 flex-auto justify-center">
                  ...
                  <h2 className="text-xl font-bold py-4 text-gray-500">
                    Purchase shield potion
                  </h2>
                  <div className="flex flex-col items-center justify-center">
                    <h3 className="text-lg font-semibold text-gray-500 mb-4">
                      {connected ? "Connected wallet" : "Connect your wallet"}
                    </h3>
                    <TonConnectButton />
                    {connected ? (
                      <div className="p-3 mt-2 text-center space-x-4 md:block">
                        <button
                          type="button"
                          className="mb-2 me-2 w-32 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
                          onClick={() => handleFirstShieldPotion(7)}
                        >
                          7 days / 0.003 TON
                        </button>
                        <button
                          type="button"
                          className="mb-2 me-2 w-32 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
                          onClick={() => handleFirstShieldPotion(14)}
                        >
                          14 days / 0.006 TON
                        </button>
                        <button
                          type="button"
                          className="mb-2 me-2 w-32 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
                          onClick={() => handleFirstShieldPotion(21)}
                        >
                          21 days / 0.009 TON
                        </button>
